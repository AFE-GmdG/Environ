...
export class Rect {
  ...
  get right(): number {
    return Number.isFinite(this.width) ? this.x + this.width : Infinity;
  }

  get bottom(): number {
    return Number.isFinite(this.height) ? this.y + this.height : Infinity;
  }
  ...
  constructor(x: number | Point | Size = 0, y: number | Point | Size = 0, width: number = 0, height: number = 0) {
    if (x instanceof Size) {
      ...
      if (Number.isNaN(x) || typeof y !== "number" || Number.isNaN(y) || !Number.isFinite(x) || !Number.isFinite(y)) {
